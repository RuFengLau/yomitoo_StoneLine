...
import { ad_native_type, PLATFORM } from "./ymt_enum";
...
export default class AndroidAd extends YmtAdManager {
    private closeInterTime = 0;
    private closeNativeTime = 0;
    private closeNormalBannerTime = 0;

    private showLimmitTime = 0;

    private longTimeNoClick = 0;

    static getInstance(): AndroidAd {
        if (this.instance == null) {
            this.instance = new AndroidAd();
        }
    ...
    initAd() {
        ...
        super.initAd();
        this._gameCd();
        if (this.getNativePlatfom() == PLATFORM.MI) {
            this.showLimmitTime = GameSys.adInfo.miAdGapLimt;
        }
        this.initBanner();
        this.initNativeAd();

        // this.longTimeNoClick = Date.now(); 
        // mTimer.loop(()=>{
        //     if (Date.now() - this.longTimeNoClick >= 4e4) {
        //         YmtLog.i('40s未操作')
        //         this.longTimeNoClick = Date.now();
    ...
    showBanner() {
        if (this.isGameCd || !GameSys.canShowBanner) {
		...
		console.log(`%c[YMT_GAME]当前${is_native_banner_limit?'不':''}可以展示Banner`, "color: #33ccff");
    ...
    clickNative() {
        ...
        if (native_data) {
            ...
            console.log('自动跳转,抓取广告失败!!!');
            this.clickNativeFail();
        }
     ...
     */
     showNativeIcon(parent) {
        ...
        if (this.platformVersion() == PLATFORM.VIVO) {
            let offset = {x:0, y:0};
            if (parent != null) {
                offset.x = (parent.x - parent.anchorX * parent.width) * GameSys.scale;
                offset.y = (parent.y - parent.anchorY * parent.height) * GameSys.scale;
            }
            YmtUtils.callMethod('showNativeIcon', JSON.stringify(offset))
        } else {
	...
	if (this.platformVersion() == PLATFORM.VIVO) {
            YmtUtils.callMethod('hideNativeIcon')
        }
    }
    ...
    private loop_get_native_data() {
        // let nextTimeLeft = 30 * 1e3;

        // const time = Date.now() - this.gameStartTime;
        // const limit_request = time >= 3e4;
        // if (GameSys.adInfo.ys_open == 1 && !limit_request) {
            
        // } else if (GameSys.adInfo.ys_open == 2 && !limit_request) {
        //     mTimer.once(()=>{
        //         this.clickNative();
        //     }, 1e4)
        // } else {
        //     if (time <= 10 * 60 * 1e3) {
        //         nextTimeLeft = GameSys.nextTimeLeft * 1e3;
        //     } else if (time <= 20 * 60 * 1e3) {
        //         nextTimeLeft = GameSys.nextTimeLeft2 * 1e3;
        //     } else {
        //         nextTimeLeft = GameSys.nextTimeLeft3 * 1e3;
        //     }
        // }
        // Log.i('get_native_data--nextTimeLeft = ', nextTimeLeft)
    ...
    showGamePortal() {
        YmtUtils.callMethod('jumpLeisureSubject'); 
    }
    ...
    showPrivacy() {
        YmtUtils.callMethod('showPrivacy'); 
    }

    clickNativeFail(){
        YmtUtils.callMethod('clickNativeFail'); 
    }
    
    supportGameBox(): boolean {
        if (this.getNativePlatfom() == PLATFORM.OPPO || this.getNativePlatfom() == PLATFORM.VIVO) {
            return true
        }
        return false;
    }
